...
import com.ejercicio.zonadetalentos.usecases.cyclist.GetCyclistsByNationalityUseCase;
...
@Configuration
public class GetAllCyclistsByNationalityRouter {
    @Bean
    public RouterFunction<ServerResponse> getCyclistsByNationality(GetCyclistsByNationalityUseCase getCyclistsByNationalityUseCase) {
        return route(GET("/api/v1/cyclist/nationality/{nationality}"), request ->
                ServerResponse
                        .ok()
                        .contentType(MediaType.APPLICATION_JSON)
                        .body(BodyInserters.fromPublisher(getCyclistsByNationalityUseCase.get(request.pathVariable("nationality")), CyclistDTO.class))
